...
CSV_PATH = os.path.join(REPO_ROOT, "telemetry", "telemetry_cognitive.csv")
...
if os.path.exists(CSV_PATH):
    ...
    with open(CSV_PATH, "r") as f:
        ...
        for row in reader:
            if row["condition"] != "Entropy-Gated (INT4/FP16)":
                continue  # baseline rows have no int4_fraction
            rows[row["prompt_label"]] = row  # keep last trial per prompt

    for cat, row in rows.items():
        categories.append(cat)
        throughputs.append(float(row["throughput_tok_sec"]))
        energies.append(float(row["joules_per_token"]))
        high_gears.append(float(row["int4_fraction"]) * 100)

if not categories:
    print("❌ No data found in telemetry/telemetry_cognitive.csv. Run cognitive_benchmark.py first.")
    exit(1)
